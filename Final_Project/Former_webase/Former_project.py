...
def infect_students_in_classroom(identifier: any, date: datetime, status: str):
    x = check_classroom(identifier)
    infected_id = classrooms[x].students
    to_infect = []

    for i in range(len(infected_id)):
        to_infect.append(infected_id[i].student_id)

    for i in range(len(students)):
        if students[i].student_id in to_infect:
            students[i].infection_date = date
            students[i].status = status
            students_in_mult_classrooms(students[i].student_id, date)
    update_data_students(students)


def students_in_mult_classrooms(identifier: int, date: datetime):
    for i in range(len(classrooms)):
        for j in range(len(classrooms[i].students)):
            if classrooms[i].students[j].student_id == identifier:
                # classrooms[i].students[j].status = status
                classrooms[i].students[j].infection_date = date

                if timedelta(days=14) > datetime.now() - classrooms[i].students[j].infection_date:
                    classrooms[i].students[j].status = "quarantined"
                    classrooms[i].students[j].infection_date = date
                    update_data_students(students)
                else:
                    classrooms[i].students[j].status = "healthy"
                    update_data_students(students)
...
def convert_student_IDs(student_ids: list):
    ...
    for x in range(len(students)):
        if students[x].student_id in student_ids:
...
def students_create(first_name: str, last_name: str, email: str,
    ...
    if students == []:
        ...
        load_data_students()
        for x in range(len(students)):
            if (str(students[x].email) == str(email)):
                print(students)
                raise HTTPException(422, "Duplicate entry: Students cannot have the same email")

        if students[-1].student_id >=1:
            # only execute this if exception has not been raised and function has not returned yet, then add the student onto the list
            stu = Student(first_name, last_name, email, phone, infection_date)
            students.append(stu)
            students[-1].student_id = students[-2].student_id + 1
            #print(students)
            update_data_students(students)
            return "StudentID: " + str(students[students.index(stu)].student_id)
...
def class_attendance(identifier: str, date: datetime):
    ...
    for i in range(len(temp)):
        if temp[i].infection_date == None or timedelta(days=14) < date - temp[i].infection_date:
            present += 1
            present_l[0] = present
    ...
    if infection_type[0] == 'c':
        ...
        for i in range(len(members)):
            ...
            if timedelta(days=14) > datetime.now() - members[i].infection_date:
                ...
                infect_students_in_classroom(identifier.upper(), date, "quarantined")
                update_data_students(students)
            else:
                members[i].status = "healthy"
                infect_students_in_classroom(identifier.upper(), date, "healthy")
                update_data_students(students)

        update_data_classrooms(classrooms)
        return f"Outbreak for Classroom {identifier.upper()} has been set infection dates have been set: {date}"
        ...
        for i in range(len(students)):
            if students[i].student_id == int(identifier):
                ...
                if timedelta(days=14) > datetime.now() - students[i].infection_date:
                    ...
                    students_in_mult_classrooms(int(identifier), date)
                    update_data_students(students)
