...
def determinant_calculator(mat):
    ...
    for i in range(len(mat)):          #We will fix and row or column and work through all of the entries, this will iterate through the elements in the row or column
        new_mat = eliminate_row_column(mat, i, 0)      #We will need to eliminate the row and column
        sign = (-1) ** (i + 1 + 1)                       #We need to set the sign as (-1) ** (row + column)
        for n in range(len(mat)):
            holder.append(mat[n][0])
        if len(new_mat) == 2:
            det += sign*two_by_two_determinant(new_mat)*holder[i]
        else:
            determinant_calculator(new_mat)              #We need to calculate the subdeterminant and then add the sign times the entry times the submatrix to the determinant
